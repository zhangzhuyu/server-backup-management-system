...
import com.ly.cloud.auth.service.ISysUserService;
...
public class SysUserServiceImpl implements ISysUserService {
    ...
    public String selectUserRoleGroup(String userName) {
        ...
        return list.stream().map(SysRole::getRole_name).collect(Collectors.joining(","));
    }
    ...
    public int updateUser(SysUser user) {
        ...
        // 删除用户与角色关联
        userRoleMapper.deleteUserRoleByuser_id(user_id);
        // 新增用户与角色管理
        insertUserRole(user);
    ...
    public void insertUserAuth(Long user_id, Long[] role_ids) {
        userRoleMapper.deleteUserRoleByuser_id(user_id);
        insertUserRole(user_id, role_ids);
    }
    ...
    public void insertUserRole(SysUser user) {
        Long[] roles = user.getrole_ids();
        if (StringUtils.isNotNull(roles)) {
            // 新增用户与角色管理
            List<SysUserRole> list = new ArrayList<SysUserRole>();
            for (Long role_id : roles) {
                SysUserRole ur = new SysUserRole();
                ur.setUserId(user.getUserId());
                ur.setUserId(role_id);
                list.add(ur);
            }
            if (list.size() > 0) {
     ...
     * @param role_ids 角色组
     */
    public void insertUserRole(Long user_id, Long[] role_ids) {
        if (StringUtils.isNotNull(role_ids)) {
            // 新增用户与角色管理
            List<SysUserRole> list = new ArrayList<SysUserRole>();
    ...
    public int deleteUserByIds(Long[] user_ids) {
        for (Long user_id : user_ids) {
            checkUserAllowed(new SysUser(user_id));
            checkUserDataScope(user_id);
        }
        // 删除用户与角色关联
        userRoleMapper.deleteUserRole(user_ids);
        return userMapper.deleteUserByIds(user_ids);
    }
...
//                    user.setPassword(Sm4Util.sm4EcbEncrypt(password));
