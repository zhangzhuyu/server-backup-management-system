package com.ly.cloud.portainer;

import com.ly.cloud.portainer.model.ErrorResponse;
import com.ly.cloud.portainer.model.PortainerEndpoint;
import com.ly.cloud.portainer.model.PortainerEndpoints;
import com.ly.cloud.portainer.model.PortainerException;
import com.ly.cloud.portainer.model.rest.*;
...
/*        client = ClientBuilder.newClient(defaultConfig);
        ...
        WebTarget authWebTarget = webTarget.path("auth");
        Invocation.Builder invocationBuilder = authWebTarget.request(MediaType.APPLICATION_JSON);
        Response response = invocationBuilder.post(Entity.entity(new Credentials(user, password), MediaType.APPLICATION_JSON));
    ...
    public PortainerEndpoints getEndpoints() throws PortainerException {
        WebTarget authWebTarget = webTarget.path("endpoints");
        Invocation.Builder invocationBuilder = authWebTarget.request(MediaType.APPLICATION_JSON).header("Authorization", "Bearer " + getJwt());
    ...
    public List<User> getUsers() throws PortainerException {
        WebTarget authWebTarget = webTarget.path("users");
        Invocation.Builder invocationBuilder = authWebTarget.request(MediaType.APPLICATION_JSON).header("Authorization", "Bearer " + getJwt());
        Response response = invocationBuilder.get();
        checkForError(response);
    ...
    public User addUser(String username, String password, Integer role) throws PortainerException {
        WebTarget authWebTarget = webTarget.path("users");
        Invocation.Builder invocationBuilder = authWebTarget.request(MediaType.APPLICATION_JSON).header("Authorization", "Bearer " + getJwt());
        Response response = invocationBuilder.post(Entity.entity(new User(username, password, role), MediaType.APPLICATION_JSON));
        checkForError(response);
    ...
    public void delUser(Long id) {
        WebTarget authWebTarget = webTarget.path("users/" + id);
        Invocation.Builder invocationBuilder = authWebTarget.request(MediaType.APPLICATION_JSON).header("Authorization", "Bearer " + getJwt());
        Response response = invocationBuilder.delete();
        System.out.println(response);
    }

    public String exec() throws PortainerException {
        WebTarget authWebTarget = webTarget.path("endpoints/10/docker/containers/6141f471e8584d7af7d132af3ec19cc8bdec1a4eacf659208eb90b4a3ab013f0/exec");
        Invocation.Builder invocationBuilder = authWebTarget.request(MediaType.APPLICATION_JSON).header("Authorization", "Bearer " + getJwt())
                .header("X-PortainerAgent-Target", "ismp");
        Response response = invocationBuilder.post(Entity.entity(new ExecParams("6141f471e8584d7af7d132af3ec19cc8bdec1a4eacf659208eb90b4a3ab013f0"), MediaType.APPLICATION_JSON));
        checkForError(response);
        ExecParams execParams = response.readEntity(new GenericType<ExecParams>() {
        });
        System.out.println(execParams);
        return execParams.getWebsocketId();
    }
}
